...
int push(ParsedFile *list, char *to_push){
    if(!list || !to_push){ //se gli argomenti non sono validi ritorno errore
        errno = EINVAL;
        return -1;
    }

    TokenizedFile *newNode = (TokenizedFile *)smalloc(sizeof(TokenizedFile));
    strcpy(newNode->word, to_push);//copio nel nodo il token
...
TokenizedFile *pop(ParsedFile *list){
    ...
    TokenizedFile *to_pop = list->head;
...
int tokenizer_r(ParsedFile *pF, FILE *fd){
    ...
    while(fgets(buf, MAX_STRING_LEN, fd)){//leggo finchè non raggiungo EOF
        ...
        while(token){//finchè ho nuovi token ciclo
            push(pF, token);//inserisco dentro a pF il nodo appena creato
...
int deleteParsedFile(ParsedFile *pF){
    ...
    TokenizedFile* tmp;//creo un puntatore temporaneo
...
ConfigFile* configFileCheck(ParsedFile *config){
    TokenizedFile *tmpPtr = config->head;//mantengo un puntatore temporaneo alla testa
    ...
    while(tmpPtr != NULL){//finchè la lista ha elementi ciclo
        ...
        switch(configOpt){ //switch sul tipo di opzione per distinguere i casi:
            ...
            default: //nel caso di elementi indesiderati nel file ritorno NULL stampando errore:
                fprintf(stderr, "ERRORE: file di config non valido\n");
                return NULL;
        }
    }
